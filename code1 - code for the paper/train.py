...
def train():
    ...
    for cs in range(len(model_list_name)):
        for data_index in range(len(dataset_name)):
            ...
            optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=opt.learning_rate, momentum = opt.momentum,weight_decay=opt.weight_decay)
            # optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=0.00001, weight_decay=0.001)
